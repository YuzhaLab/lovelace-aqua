...
import {
  PUMP_CARD_EDITOR_NAME,
  PUMP_CARD_NAME,
  PUMP_ENTITY_DOMAINS,
} from "./const";
// import "./controls/fan-oscillate-control";
// import "./controls/fan-percentage-control";
import { FanCardConfig } from "./pump-card-config";
import { getPercentage } from "./utils";

registerCustomCard({
  type: PUMP_CARD_NAME,
  name: "Aqua Pump Card",
  description: "Flow and pump cards",
});

@customElement(PUMP_CARD_NAME)
export class AquaFanCard
  extends MushroomBaseCard<FanCardConfig>
  implements LovelaceCard {
  public static async getConfigElement(): Promise<LovelaceCardEditor> {
    await import("./pump-card-editor");
    return document.createElement(PUMP_CARD_EDITOR_NAME) as LovelaceCardEditor;
  }
  ...
  public static async getStubConfig(
    ...
    const fans = entities.filter((e) =>
        PUMP_ENTITY_DOMAINS.includes(e.split(".")[0])
    );
    return {
      type: `custom:${PUMP_CARD_NAME}`,
      entity: fans[0],
    };
  }
  ...
  protected render() {
    ...
    const stateObj = this._stateObj;
    const stateObj1 = this._stateObj1;
    ...
    return html`
      ...
      hasDoubleClick: hasAction(this._config.double_tap_action),
        ...
        : this.renderIcon(stateObj, icon)}
            ...
            ${this.renderStateInfo(stateObj, appearance, name, stateDisplay,stateObj1)};
          </mushroom-state-item>
          ${displayControls
        ? html`
